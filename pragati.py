...
# =========================================================================
# Strategy 1: RegimeAdaptiveAlpha
# =========================================================================

class RegimeAdaptiveAlpha(BaseStrategy):
    """
    Regime-Adaptive Alpha Generator
    
    Edge vs VolatilitySurfer: Dynamic factor rotation based on market regime
    - VolatilitySurfer uses static breakout logic
    - This rotates between momentum/value/defensive based on conditions
    """
    ...
    def generate_portfolio(self, df: pd.DataFrame, sip_amount: float = 100000.0) -> pd.DataFrame:
        ...
        regime_scores = self._calculate_regime_probabilities(df)
        df = self._calculate_momentum_factor(df)
        df = self._calculate_value_factor(df)
        df = self._calculate_quality_factor(df)
        df = self._calculate_defensive_factor(df)
        df['composite_score'] = self._blend_factors_by_regime(df, regime_scores)
        df['composite_score'] = df['composite_score'] * df['risk_adjustment'] + 1e-6
        
        total_score = df['composite_score'].sum()
        df['weightage'] = df['composite_score'] / total_score if total_score > 0 else 1 / len(df)
        return self._allocate_portfolio(df, sip_amount)
    
    def _calculate_regime_probabilities(self, df):
        avg_rsi = df['rsi latest'].mean()
        avg_osc = df['osc latest'].mean()
        avg_vol = (df['dev20 latest'] / df['price']).mean()
        rsi_dispersion = df['rsi latest'].std()
        above_ma200_pct = (df['price'] > df['ma200 latest']).mean()
        ma_alignment_pct = (df['ma90 latest'] > df['ma200 latest']).mean()
        
        trend_score = (avg_rsi - 50) / 25 + avg_osc / 50 + (above_ma200_pct - 0.5) * 2 + (ma_alignment_pct - 0.5) * 2
        p_trending = 1 / (1 + np.exp(-trend_score))
        
        reversion_score = rsi_dispersion / 15 + (1 - abs(avg_rsi - 50) / 25)
        p_reversion = 1 / (1 + np.exp(-reversion_score + 1))
        
        crisis_score = (35 - avg_rsi) / 15 + (-40 - avg_osc) / 30 + (avg_vol - 0.025) * 20
        p_crisis = 1 / (1 + np.exp(-crisis_score))
        
        total_p = p_trending + p_reversion + p_crisis
        return {'trending': p_trending / total_p, 'reversion': p_reversion / total_p, 'crisis': p_crisis / total_p}
    
    def _calculate_momentum_factor(self, df):
        price_mom = (df['price'] / df['ma90 latest'] - 1) * 0.5 + (df['price'] / df['ma200 latest'] - 1) * 0.5
        rsi_mom = (df['rsi latest'] - 50) / 50 * 0.6 + (df['rsi weekly'] - 50) / 50 * 0.4
        osc_mom = df['osc latest'] / 100 * 0.5 + df['osc weekly'] / 100 * 0.5
        ema_accel = np.where(df['9ema osc latest'] > df['21ema osc latest'], 
                            (df['9ema osc latest'] - df['21ema osc latest']) / 50, -0.5)
        raw = price_mom * 0.3 + rsi_mom * 0.25 + osc_mom * 0.25 + ema_accel * 0.2
        min_val, max_val = raw.min(), raw.max()
        df['momentum_factor'] = (raw - min_val) / (max_val - min_val) if max_val > min_val else 0.5
        return df
    
    def _calculate_value_factor(self, df):
        zscore_value = np.clip(-df['zscore latest'] / 2, 0, 2)
        zscore_weekly_value = np.clip(-df['zscore weekly'] / 2, 0, 2)
        rsi_value = np.clip((40 - df['rsi latest']) / 20, 0, 2)
        ma_distance = np.clip((df['ma200 latest'] - df['price']) / df['ma200 latest'], 0, 0.3) * 5
        bb_lower = df['ma20 latest'] - 2 * df['dev20 latest']
        bb_distance = np.clip((bb_lower - df['price']) / df['ma20 latest'], 0, 0.2) * 5
        raw = zscore_value * 0.3 + zscore_weekly_value * 0.2 + rsi_value * 0.2 + ma_distance * 0.15 + bb_distance * 0.15
        min_val, max_val = raw.min(), raw.max()
        df['value_factor'] = (raw - min_val) / (max_val - min_val) if max_val > min_val else 0.5
        return df
    
    def _calculate_quality_factor(self, df):
        trend_quality = np.where((df['price'] > df['ma90 latest']) & (df['ma90 latest'] > df['ma200 latest']), 1.5,
                                np.where(df['price'] > df['ma200 latest'], 1.0, 0.5))
        alignment = np.where(np.sign(df['osc latest']) == np.sign(df['osc weekly']), 1.2, 0.8)
        rsi_stability = np.where((df['rsi latest'] > 35) & (df['rsi latest'] < 65), 1.2,
                                np.where((df['rsi latest'] > 25) & (df['rsi latest'] < 75), 1.0, 0.7))
        raw = trend_quality * alignment * rsi_stability
        min_val, max_val = raw.min(), raw.max()
        df['quality_factor'] = (raw - min_val) / (max_val - min_val) if max_val > min_val else 0.5
        return df
    
    def _calculate_defensive_factor(self, df):
        vol_normalized = df['dev20 latest'] / df['price']
        vol_score = 1 - np.clip(vol_normalized / 0.05, 0, 1)
        support_distance = np.abs(df['price'] - df['ma200 latest']) / df['price']
        support_score = 1 - np.clip(support_distance / 0.2, 0, 1)
        rsi_moderate = 1 - np.abs(df['rsi latest'] - 50) / 50
        raw = vol_score * 0.4 + support_score * 0.3 + rsi_moderate * 0.3
        min_val, max_val = raw.min(), raw.max()
        df['defensive_factor'] = (raw - min_val) / (max_val - min_val) if max_val > min_val else 0.5
        df['risk_adjustment'] = np.where(vol_normalized > 0.04, 0.7, np.where(vol_normalized > 0.025, 0.9, 1.1))
        return df
    
    def _blend_factors_by_regime(self, df, regime_scores):
        weights = {
            'trending': {'momentum': 0.50, 'value': 0.10, 'quality': 0.30, 'defensive': 0.10},
            'reversion': {'momentum': 0.15, 'value': 0.45, 'quality': 0.25, 'defensive': 0.15},
            'crisis': {'momentum': 0.05, 'value': 0.30, 'quality': 0.20, 'defensive': 0.45}
        }
        blended_mom_w = sum(regime_scores[r] * weights[r]['momentum'] for r in regime_scores)
        blended_val_w = sum(regime_scores[r] * weights[r]['value'] for r in regime_scores)
        blended_qual_w = sum(regime_scores[r] * weights[r]['quality'] for r in regime_scores)
        blended_def_w = sum(regime_scores[r] * weights[r]['defensive'] for r in regime_scores)
        return (df['momentum_factor'] * blended_mom_w + df['value_factor'] * blended_val_w +
                df['quality_factor'] * blended_qual_w + df['defensive_factor'] * blended_def_w)


# =========================================================================
# Strategy 2: StatisticalEdgeHunter
# =========================================================================

class StatisticalEdgeHunter(BaseStrategy):
    """
    Statistical Edge Hunter
    
    Edge vs VolatilitySurfer: Pure statistical anomaly exploitation
    - Uses cross-sectional z-scores and percentile ranks
    ...
    def generate_portfolio(self, df: pd.DataFrame, sip_amount: float = 100000.0) -> pd.DataFrame:
        ...
        df = self._calculate_cross_sectional_zscores(df)
        df = self._calculate_ts_anomaly_scores(df)
        df = self._calculate_signal_confluence(df)
        df = self._calculate_edge_score(df)
        
        df['composite_score'] = df['edge_score'] * df['confidence_multiplier'] + 1e-6
        total_score = df['composite_score'].sum()
        df['weightage'] = df['composite_score'] / total_score if total_score > 0 else 1 / len(df)
        return self._allocate_portfolio(df, sip_amount)
    
    def _calculate_cross_sectional_zscores(self, df):
        def cs_zscore(series, clip=3.0):
            mean, std = series.mean(), series.std()
            return np.clip((series - mean) / std, -clip, clip) if std > 0 else pd.Series([0.0] * len(series), index=series.index)
        
        df['cs_rsi_z'] = cs_zscore(df['rsi latest'])
        df['cs_osc_z'] = cs_zscore(df['osc latest'])
        df['price_mom'] = df['price'] / df['ma90 latest'] - 1
        df['cs_price_mom_z'] = cs_zscore(df['price_mom'])
        df['vol_normalized'] = df['dev20 latest'] / df['price']
        df['cs_vol_z'] = cs_zscore(df['vol_normalized'])
        return df
    
    def _calculate_ts_anomaly_scores(self, df):
        df['ts_anomaly'] = df['zscore latest'] * 0.55 + df['zscore weekly'] * 0.45
        df['anomaly_category'] = np.select(
            [df['ts_anomaly'] < -2.5, df['ts_anomaly'] < -2.0, df['ts_anomaly'] < -1.5,
             df['ts_anomaly'] < -1.0, df['ts_anomaly'] > 2.0, df['ts_anomaly'] > 1.5],
            [3.0, 2.5, 2.0, 1.5, 0.3, 0.5], default=1.0)
        return df
    
    def _calculate_signal_confluence(self, df):
        signals = pd.DataFrame(index=df.index)
        signals['cs_rsi_oversold'] = (df['cs_rsi_z'] < -1.0).astype(int)
        signals['cs_osc_oversold'] = (df['cs_osc_z'] < -1.0).astype(int)
        signals['cs_low_vol'] = (df['cs_vol_z'] < 0).astype(int)
        signals['ts_oversold'] = (df['ts_anomaly'] < -1.5).astype(int)
        signals['ts_weekly_confirm'] = (df['zscore weekly'] < -1.0).astype(int)
        signals['rsi_oversold'] = (df['rsi latest'] < 35).astype(int)
        signals['osc_deeply_negative'] = (df['osc latest'] < -50).astype(int)
        signals['ema_turning'] = (df['9ema osc latest'] > df['21ema osc latest']).astype(int)
        signals['price_below_ma200'] = (df['price'] < df['ma200 latest']).astype(int)
        df['signal_count'] = signals.sum(axis=1)
        df['confluence_score'] = np.sqrt(df['signal_count'] / 9) * 2
        return df
    
    def _calculate_edge_score(self, df):
        statistical_score = (-df['cs_rsi_z'] * 0.15 + -df['cs_osc_z'] * 0.20 + 
                            df['anomaly_category'] * 0.30 + df['confluence_score'] * 0.35)
        min_val, max_val = statistical_score.min(), statistical_score.max()
        df['base_edge'] = (statistical_score - min_val) / (max_val - min_val) if max_val > min_val else 0.5
        
        df['momentum_confirm'] = np.where(df['9ema osc latest'] > df['21ema osc latest'], 1.3,
                                          np.where(df['9ema osc latest'] > df['9ema osc latest'].shift(1).fillna(0), 1.0, 0.7))
        df['edge_score'] = df['base_edge'] * df['anomaly_category'] * df['momentum_confirm']
        df['confidence_multiplier'] = np.select(
            [df['signal_count'] >= 6, df['signal_count'] >= 4, df['signal_count'] >= 2],
            [1.4, 1.2, 1.0], default=0.7)
        return df


# =========================================================================
# Strategy 3: MultiTimeframeSynergy
# =========================================================================

class MultiTimeframeSynergy(BaseStrategy):
    """
    Multi-Timeframe Synergy
    
    Edge vs VolatilitySurfer: Exploits daily/weekly divergences and convergences
    - Better signal quality through multi-timeframe confirmation
    - Earlier entry through divergence detection
    """
    ...
    def generate_portfolio(self, df: pd.DataFrame, sip_amount: float = 100000.0) -> pd.DataFrame:
        ...
        df = self._calculate_alignment_scores(df)
        df = self._calculate_divergence_signals(df)
        df = self._calculate_information_weighted_score(df)
        df = self._apply_synergy_multiplier(df)
        
        df['composite_score'] = df['final_mtf_score'] + 1e-6
        total_score = df['composite_score'].sum()
        df['weightage'] = df['composite_score'] / total_score if total_score > 0 else 1 / len(df)
        return self._allocate_portfolio(df, sip_amount)
    
    def _calculate_alignment_scores(self, df):
        df['rsi_alignment'] = np.where(((df['rsi latest'] > 50) & (df['rsi weekly'] > 50)) |
                                       ((df['rsi latest'] < 50) & (df['rsi weekly'] < 50)), 1.0, -0.5)
        df['osc_alignment'] = np.where(np.sign(df['osc latest']) == np.sign(df['osc weekly']), 1.0, -0.5)
        daily_ema_bull = df['9ema osc latest'] > df['21ema osc latest']
        weekly_ema_bull = df['9ema osc weekly'] > df['21ema osc weekly']
        df['ema_alignment'] = np.where(daily_ema_bull == weekly_ema_bull, 1.0, -0.3)
        df['zscore_alignment'] = np.where(np.sign(df['zscore latest']) == np.sign(df['zscore weekly']), 1.0, -0.3)
        df['alignment_score'] = (df['rsi_alignment'] * 0.25 + df['osc_alignment'] * 0.30 +
                                df['ema_alignment'] * 0.25 + df['zscore_alignment'] * 0.20)
        return df
    
    def _calculate_divergence_signals(self, df):
        df['rsi_divergence'] = np.where((df['rsi latest'] > df['rsi weekly']) & (df['rsi weekly'] < 40),
                                        (df['rsi latest'] - df['rsi weekly']) / 20, 0)
        df['osc_divergence'] = np.where((df['osc latest'] > df['osc weekly']) & (df['osc weekly'] < -30),
                                        (df['osc latest'] - df['osc weekly']) / 50, 0)
        df['ema_divergence'] = np.where((df['9ema osc latest'] > df['21ema osc latest']) & 
                                        (df['9ema osc weekly'] <= df['21ema osc weekly']), 1.5,
                                        np.where((df['9ema osc latest'] <= df['21ema osc latest']) &
                                                (df['9ema osc weekly'] > df['21ema osc weekly']), 0.7, 1.0))
        df['divergence_score'] = df['rsi_divergence'] * 0.30 + df['osc_divergence'] * 0.35 + (df['ema_divergence'] - 1) * 0.35
        return df
    
    def _calculate_information_weighted_score(self, df):
        market_oversold = df['osc weekly'].mean() < -30
        daily_weight, weekly_weight = (0.35, 0.65) if market_oversold else (0.55, 0.45)
        
        df['daily_score'] = ((50 - df['rsi latest']) / 50 * 0.25 + (-df['osc latest']) / 100 * 0.30 +
                            (-df['zscore latest']) / 3 * 0.25 + 
                            np.where(df['9ema osc latest'] > df['21ema osc latest'], 0.5, 0) * 0.20)
        df['weekly_score'] = ((50 - df['rsi weekly']) / 50 * 0.25 + (-df['osc weekly']) / 100 * 0.30 +
                             (-df['zscore weekly']) / 3 * 0.25 +
                             np.where(df['9ema osc weekly'] > df['21ema osc weekly'], 0.5, 0) * 0.20)
        df['info_weighted_score'] = df['daily_score'] * daily_weight + df['weekly_score'] * weekly_weight
        return df
    
    def _apply_synergy_multiplier(self, df):
        aligned = (df['alignment_score'] > 0.5).astype(int) + (df['divergence_score'] > 0).astype(int)
        df['synergy_mult'] = np.where(aligned >= 2, 1.5, np.where(aligned >= 1, 1.2, 
                                      np.where(df['alignment_score'] < -0.3, 0.7, 1.0)))
        df['trend_filter'] = np.where(df['price'] > df['ma200 latest'], 1.1,
                                      np.where((df['price'] > df['ma90 latest']) | (df['zscore latest'] < -1.5), 1.0, 0.8))
        df['final_mtf_score'] = df['info_weighted_score'] * df['synergy_mult'] * df['trend_filter'] * (1 + df['divergence_score'])
        min_val = df['final_mtf_score'].min()
        if min_val < 0:
            df['final_mtf_score'] = df['final_mtf_score'] - min_val + 0.1
        return df


# =========================================================================
# Strategy 4: AdaptiveRiskParity
# =========================================================================

class AdaptiveRiskParity(BaseStrategy):
    """
    Adaptive Risk Parity
    
    Edge vs VolatilitySurfer: Volatility-normalized equal risk contribution
    - Better risk-adjusted returns through volatility normalization
    - Avoids concentration in high-volatility names
    """
    ...
    def generate_portfolio(self, df: pd.DataFrame, sip_amount: float = 100000.0) -> pd.DataFrame:
        ...
        df = self._calculate_risk_parity_weights(df)
        df = self._apply_market_adjustment(df)
        
        df['composite_score'] = df['risk_parity_score'] + 1e-6
        total_score = df['composite_score'].sum()
        df['weightage'] = df['composite_score'] / total_score if total_score > 0 else 1 / len(df)
        return self._allocate_portfolio(df, sip_amount)
    
    def _calculate_risk_scores(self, df):
        df['realized_vol'] = df['dev20 latest'] / df['price']
        df['weekly_vol'] = df['dev20 weekly'] / df['price']
        df['blended_vol'] = df['realized_vol'] * 0.6 + df['weekly_vol'] * 0.4
        df['vol_rank'] = df['blended_vol'].rank(pct=True)
        df['inv_vol_score'] = 1 / np.maximum(df['blended_vol'], 0.001)
        return df
    
    def _calculate_opportunity_scores(self, df):
        df['zscore_opp'] = np.clip(-df['zscore latest'] / 2, 0, 2.5)
        df['zscore_weekly_opp'] = np.clip(-df['zscore weekly'] / 2, 0, 2.5)
        df['rsi_opp'] = np.clip((40 - df['rsi latest']) / 20, 0, 2)
        df['osc_opp'] = np.clip(-df['osc latest'] / 50, 0, 2)
        df['turn_signal'] = np.where((df['9ema osc latest'] > df['21ema osc latest']) & (df['osc latest'] < 0), 1.3,
                                     np.where(df['9ema osc latest'] > df['21ema osc latest'], 1.1, 0.9))
        df['opportunity_score'] = (df['zscore_opp'] * 0.30 + df['zscore_weekly_opp'] * 0.20 + 
                                  df['rsi_opp'] * 0.20 + df['osc_opp'] * 0.20 + (df['turn_signal'] - 1) * 0.10) + 1
        return df
    
    def _calculate_risk_parity_weights(self, df):
        total_inv_vol = df['inv_vol_score'].sum()
        df['risk_parity_base'] = df['inv_vol_score'] / total_inv_vol if total_inv_vol > 0 else 1 / len(df)
        opp_mean = df['opportunity_score'].mean()
        df['opportunity_tilt'] = df['opportunity_score'] / opp_mean if opp_mean > 0 else 1.0
        df['risk_parity_score'] = df['risk_parity_base'] * df['opportunity_tilt']
        df['risk_parity_score'] = np.where(df['vol_rank'] > 0.8, df['risk_parity_score'] * 0.7, df['risk_parity_score'])
        return df
    
    def _apply_market_adjustment(self, df):
        avg_osc, avg_rsi = df['osc latest'].mean(), df['rsi latest'].mean()
        if avg_osc < -40 and avg_rsi < 40:
            df['risk_parity_score'] = df['risk_parity_score'] * (df['opportunity_score'] / df['opportunity_score'].mean())
        elif avg_osc > 30 and avg_rsi > 60:
            df['risk_parity_score'] = df['risk_parity_base'] * 1.5
        df['quality_filter'] = np.where(df['price'] > df['ma200 latest'], 1.1,
                                        np.where(df['ma90 latest'] > df['ma200 latest'], 1.0, 0.85))
        df['risk_parity_score'] = df['risk_parity_score'] * df['quality_filter']
        return df


# =========================================================================
# Strategy 5: MeanReversionMaster
# =========================================================================

class MeanReversionMaster(BaseStrategy):
    """
    Mean Reversion Master
    
    Edge vs VolatilitySurfer: Specializes in reversal plays with strict confirmation
    - Captures large moves from statistical extremes
    - Avoids "catching falling knives" through confirmation requirements
    """
    ...
    def generate_portfolio(self, df: pd.DataFrame, sip_amount: float = 100000.0) -> pd.DataFrame:
        ...
        df = self._calculate_reversion_opportunity(df)
        df = self._calculate_confirmation_signals(df)
        df = self._calculate_quality_score(df)
        df = self._calculate_final_score(df)
        
        df['composite_score'] = df['final_mr_score'] + 1e-6
        total_score = df['composite_score'].sum()
        df['weightage'] = df['composite_score'] / total_score if total_score > 0 else 1 / len(df)
        return self._allocate_portfolio(df, sip_amount)
    
    def _calculate_reversion_opportunity(self, df):
        df['zscore_deviation'] = np.clip(-df['zscore latest'], 0, 4)
        df['zscore_weekly_deviation'] = np.clip(-df['zscore weekly'], 0, 4)
        df['rsi_extreme'] = np.clip((35 - df['rsi latest']) / 15, 0, 2.5)
        df['rsi_weekly_extreme'] = np.clip((40 - df['rsi weekly']) / 15, 0, 2.5)
        df['osc_extreme'] = np.clip((-df['osc latest'] - 50) / 25, 0, 2.5)
        df['osc_weekly_extreme'] = np.clip((-df['osc weekly'] - 40) / 25, 0, 2.5)
        bb_lower = df['ma20 latest'] - 2 * df['dev20 latest']
        df['bb_deviation'] = np.clip((bb_lower - df['price']) / df['dev20 latest'], 0, 3)
        df['ma_deviation'] = np.clip((df['ma200 latest'] - df['price']) / df['ma200 latest'] * 10, 0, 3)
        df['reversion_opportunity'] = (df['zscore_deviation'] * 0.25 + df['zscore_weekly_deviation'] * 0.15 +
                                       df['rsi_extreme'] * 0.15 + df['rsi_weekly_extreme'] * 0.10 +
                                       df['osc_extreme'] * 0.15 + df['osc_weekly_extreme'] * 0.10 +
                                       df['bb_deviation'] * 0.05 + df['ma_deviation'] * 0.05)
        return df
    
    def _calculate_confirmation_signals(self, df):
        df['ema_turn'] = np.where(df['9ema osc latest'] > df['21ema osc latest'], 2.0, 0.8)
        df['rsi_turn'] = np.where((df['rsi latest'] > 30) & (df['rsi latest'] < 50), 1.5,
                                  np.where(df['rsi latest'] < 25, 0.7, 1.0))
        df['osc_improving'] = np.where(df['osc latest'] > df['osc weekly'], 1.3, 0.9)
        df['weekly_support'] = np.where((df['rsi weekly'] < 40) & (df['osc weekly'] < -30), 1.4,
                                        np.where(df['rsi weekly'] < 50, 1.1, 0.9))
        df['confirmation_score'] = (df['ema_turn'] * 0.35 + df['rsi_turn'] * 0.25 +
                                   df['osc_improving'] * 0.20 + df['weekly_support'] * 0.20)
        return df
    
    def _calculate_quality_score(self, df):
        df['trend_quality'] = np.where(df['ma90 latest'] > df['ma200 latest'], 1.4,
                                       np.where(df['ma90 weekly'] > df['ma200 weekly'], 1.2, 0.8))
        vol_norm = df['dev20 latest'] / df['price']
        df['vol_quality'] = np.where((vol_norm > 0.01) & (vol_norm < 0.035), 1.2,
                                     np.where(vol_norm > 0.05, 0.7, 0.9))
        df['cs_quality'] = 1 - (df['rsi latest'].rank(pct=True) * 0.5)
        df['quality_score'] = df['trend_quality'] * 0.40 + df['vol_quality'] * 0.30 + df['cs_quality'] * 0.30
        return df
    
    def _calculate_final_score(self, df):
        df['raw_mr_score'] = df['reversion_opportunity'] * df['confirmation_score'] * df['quality_score']
        df['filter_confirm'] = np.where(df['confirmation_score'] > 1.0, 1.0, 0.5)
        df['filter_freefall'] = np.where((df['osc latest'] < -90) & (df['9ema osc latest'] < df['21ema osc latest']), 0.4, 1.0)
        df['filter_quality'] = np.where(df['quality_score'] > 0.9, 1.0, 0.7)
        df['final_mr_score'] = df['raw_mr_score'] * df['filter_confirm'] * df['filter_freefall'] * df['filter_quality']
        min_score = df['final_mr_score'].min()
        if min_score < 0:
            df['final_mr_score'] = df['final_mr_score'] - min_score + 0.01
        return df
...
@st.cache_data
def load_historical_data() -> List[Tuple[datetime, pd.DataFrame]]:
    historical_dir = 'data/historical'
    if not os.path.exists(historical_dir): return []
    ...
    for f in files:
        try:
            ...
            if not df.empty: data.append((date, df))
        except Exception as e: logging.warning(f"Skipping invalid file {f}: {e}")
...
def get_market_mix_suggestion_v3(historical_data: list) -> Tuple[str, str, float, Dict]:
...
def display_performance_metrics(performance: Dict):
    ...
    for name, perf in performance.get('strategy', {}).items():
        if perf.get('returns'):
            df = pd.DataFrame(perf['returns']).set_index('date')
...
def main():
    ...
    PORTFOLIO_STYLES = {
        "Swing Trading": {
            ...
            "mixes": {
                ...
                "Chop/Consolidate Mix": {
                    "strategies": ['VolatilitySurfer', 'WormholeTemporal', 'AdaptiveVolBreakout', 'MultiverseAlpha'],
                }
            }
        },
        ...
        "🌍 All Weather": {
            ...
            "mixes": {
                "🐂 Bull Market Mix": {
                    "strategies": ['RegimeAdaptiveAlpha', 'StatisticalEdgeHunter', 'MultiTimeframeSynergy', 'VolatilitySurfer'],
                    "rationale": "SHARPE"
                },
                ...
                "📊 Chop/Consolidate Mix": {
                    "strategies": ['RegimeAdaptiveAlpha', 'MultiTimeframeSynergy', 'VolatilitySurfer'],
                    "rationale": "SORTINO"
                }
            }
        }
    ...
    if not all_historical_data:
        st.error("No historical data found in 'data/historical'. Please add historical CSV files (e.g., '2023-10-17.csv').")
        return

    historical_data_map = {date.strftime('%Y-%m-%d'): df for date, df in all_historical_data}
    available_dates = sorted(historical_data_map.keys(), reverse=True)
    ...
    with st.sidebar:
        ...
        selected_date_str = st.selectbox(
            ...
            available_dates,
            help="Choose a date from the historical data to run the portfolio curation."
        )
        ...
        # --- NEW: Use V2 of Market Mix Suggester ---
        st.markdown("### Market Condition Mix")
        
        # Get historical data for the regime detector
        regime_detection_window = [d for d in all_historical_data if d[0] <= datetime.strptime(selected_date_str, '%Y-%m-%d')]

        if regime_detection_window:
            suggested_mix, explanation, confidence, details = get_market_mix_suggestion_v3(regime_detection_window)
            st.info(f"**Suggested Mix:** {suggested_mix.split(' ')[1]} (Confidence: {confidence:.0%})", icon="💡")
            with st.expander("View Regime Analysis Details"):
                st.markdown(explanation)

            mix_options = list(PORTFOLIO_STYLES[selected_main_branch]["mixes"].keys())
            try:
                default_mix_index = mix_options.index(suggested_mix)
            except ValueError:
                default_mix_index = 0
        else:
            default_mix_index = 0
            st.warning("Could not load data to generate a suggestion.")

        selected_mix_name = st.selectbox(
            "2. Select Market Mix",
            options=list(PORTFOLIO_STYLES[selected_main_branch]["mixes"].keys()),
            index=default_mix_index,
            help="Select the market condition. A suggestion is provided based on data analysis."
        )
        
        # Display details for the selected style
        if selected_main_branch and selected_mix_name:
            branch_info = PORTFOLIO_STYLES[selected_main_branch]
            mix_info = branch_info["mixes"][selected_mix_name]
            with st.expander("View Selection Details", expanded=True):
                st.markdown(f"**Style:** {branch_info['description']}")
                st.markdown(f"**Rationale:** *{mix_info['rationale']}*")
                st.markdown(f"**Strategies Used:** `{'`, `'.join(mix_info['strategies'])}`")

        st.markdown("### Training Data Selection")
        target_date_dt = datetime.strptime(selected_date_str, '%Y-%m-%d')
        data_before_analysis = [(d, df) for d, df in all_historical_data if d < target_date_dt]
        max_lookback = len(data_before_analysis)
        ...
        if max_lookback >= 10:
            lookback_files = st.number_input(
                ...
                value=min(25, max_lookback),
                step=5,
                help=f"Number of historical files to use for training, prior to the analysis date. Max: {max_lookback}"
            )
        else:
            st.warning("Not enough history (<10 files) for a lookback period before the selected analysis date.")
        ...
        if st.button("🚀 Run Analysis", use_container_width=True, type="primary"):
            ...
            # Determine which strategies to run based on the selected style
            style_strategies = PORTFOLIO_STYLES[selected_main_branch]["mixes"][selected_mix_name]['strategies']
            strategies_to_run = {name: strategies[name] for name in style_strategies}

            current_df = historical_data_map.get(selected_date_str)
            if current_df is None or current_df.empty:
                st.error(f"No data available for the selected date: {selected_date_str}")
            else:
                st.session_state.current_df = current_df
                st.session_state.selected_date = selected_date_str

                # --- APPLY LOOKBACK FILES FILTER ---
                training_data_window = []
                if lookback_files > 0:
                    training_data_window = data_before_analysis[-lookback_files:]
                    st.info(f"Training on {len(training_data_window)} files before {selected_date_str}.")
                else:
                    st.error("Lookback period is not set or is invalid. At least 10 files are required.")
                    return

                if not training_data_window:
                    st.error("Failed to create a training data window.")
                    return
                
                st.session_state.performance = evaluate_historical_performance(strategies_to_run, training_data_window)
